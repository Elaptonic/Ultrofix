...
export default function VendorProfileScreen() {
  ...
  const menuSections: MenuRow[][] = [
    ...
    [
      ...
      { icon: "refresh-cw", label: "Switch to Consumer", sub: "Book services instead", color: colors.primary, onPress: () => setRole("consumer") },
    ],
    [
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]} contentContainerStyle={{ paddingTop: insets.top + (Platform.OS === "web" ? 20 : 8), paddingBottom: insets.bottom + 100 }} showsVerticalScrollIndicator={false}>
      ...
      <View style={[styles.statsRow, { backgroundColor: colors.card, borderColor: colors.border }]}>{[{ label: "Jobs Done", value: "248" }, { label: "Rating", value: "4.8 ★" }, { label: "Since", value: "2023" }].map((s, i) => <View key={i} style={[styles.statItem, i < 2 && { borderRightWidth: StyleSheet.hairlineWidth, borderRightColor: colors.border }]}><Text style={[styles.statValue, { color: colors.foreground }]}>{s.value}</Text><Text style={[styles.statLabel, { color: colors.mutedForeground }]}>{s.label}</Text></View>)}</View>

      {menuSections.map((section, si) => <View key={si} style={[styles.menuCard, { backgroundColor: colors.card, borderColor: colors.border }]}>{section.map((row, ri) => <View key={ri}>{ri > 0 && <View style={[styles.divider, { backgroundColor: colors.border }]} />}<Pressable onPress={row.onPress} style={({ pressed }) => [styles.menuRow, pressed && { opacity: 0.65 }]}><View style={[styles.menuIcon, { backgroundColor: row.color ? row.color + "15" : colors.muted }]}><Feather name={row.icon} size={16} color={row.color ?? colors.foreground} /></View><View style={styles.menuText}><Text style={[styles.menuLabel, { color: row.color ?? colors.foreground }]}>{row.label}</Text>{row.sub && <Text style={[styles.menuSub, { color: colors.mutedForeground }]}>{row.sub}</Text>}</View><Feather name="chevron-right" size={16} color={colors.mutedForeground} /></Pressable></View>)}</View>)}
    </ScrollView>
  );
}
